...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for position in self.legalPositions:
            ...
            # Calculate conditional probability of noisyDistance P(noisyDistance|TrueDistance)
            P = emissionModel[trueDistance]
            if P > 0:
                # Updates agent's belief sensors with P
                allPossible[position] = self.beliefs[position] * P
        # Check if Ghost has been captured 
        if noisyDistance == None:
            # If yes, update all beliefs with Jail position
            allPossible[self.getJailPosition()] = 1.0

        "*** END YOUR CODE HERE ***"

        allPossible.normalize()
        # Update self.beliefs with all possible values
